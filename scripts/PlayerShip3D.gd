	...
	match upgrade_type:
		...
		"debris_scanner":
			if level > 0:
				...
				_log_message("PlayerShip3D: Debris scanner upgrade applied - Level %d" % level)
			else:
		...
		"cargo_magnet":
			if level > 0:
				enable_cargo_magnet()
				_log_message("PlayerShip3D: Cargo magnet upgrade applied - Level %d" % level)
			else:
				is_magnet_active = false
				magnet_range = 0.0
				_log_message("PlayerShip3D: Cargo magnet deactivated")

	_log_message("PlayerShip3D: Upgrade effects applied - Speed: %.1f, Capacity: %d, Collection Range: %.1f, Scanner: %s, Magnet: %s" % [speed, inventory_capacity, collection_range, str(is_scanner_active), str(is_magnet_active)])
